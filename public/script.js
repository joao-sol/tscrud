...
form.addEventListener('submit', async (e) => {
    ...
    if (editMode) {
        // Atualiza empréstimo
        await fetch(`:3000/emprestimos/${editId}`,{
            method: 'PUT',
            headers: {
...
async function editarEmprestimo(id) {
    try {
        const response = await fetch(`:3000/emprestimos/${id}`);
        if (!response.ok) {
...
async function deletarEmprestimo(id) {
    await fetch(`:3000/emprestimos/${id}`, {
        method: 'DELETE'
    });
    carregarEmprestimos();
}
...
async function carregarEmprestimos() {
    const response = await fetch(':3000/emprestimos');
